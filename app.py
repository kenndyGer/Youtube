...
def youtube_search_without_api(query):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            # Wir würfeln die Suchbegriffe, damit bei jedem "Weiter" ein anderes Video an erster Stelle steht!
            ...
            if 'entries' in info and len(info['entries']) > 0:
                ...
                # Wechselsperre: Aktuelles Lied ausschließen
...
def alexa_endpoint():
    ...
    if request_type == "LaunchRequest":
        return jsonify({"version": "1.0", "response": {"outputSpeech": {"type": "PlainText", "text": "YouTube Suche ist bereit. Was möchtest du hören?"}, "shouldEndSession": False}})
        ...
        if intent_name in ["SearchImmediatelyIntent", "AMAZON.NextIntent"]:
            ...
            if not video_id or not audio_url:
                return jsonify({"version": "1.0", "response": {"outputSpeech": {"type": "PlainText", "text": "Ich konnte kein Lied finden."}}})
            ...
            if aktuelle_audio_url:
                return jsonify({"version": "1.0", "response": {"directives": [{"type": "AudioPlayer.Play", "playBehavior": "REPLACE_ALL", "audioItem": {"stream": {"token": aktueller_token, "url": aktuelle_audio_url, "offsetInMilliseconds": pausen_zeitpunkt}}}]}})
